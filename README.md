...
* Sort a list of integers using a limited number of operations.
* Handle different input cases and errors.
...
The project requires thinking about:

* how to represent the two stacks;
* which elements should be moved;
* the cost of rotations;
* the order in which operations should be performed;
* reducing unnecessary operations;
* adapting the strategy to the size of the input.
...
The project also taught me to analyze **algorithmic complexity**, compare different strategies, and design data structures adapted to a specific problem.
...
* Trier une liste d’entiers avec un nombre limité d’opérations.
* Gérer différents cas d’entrée et les erreurs.
...
* Parsing et validation des entrées
...
Le projet demande notamment de réfléchir à :

* la manière de représenter les deux piles ;
* la sélection des éléments à déplacer ;
* le coût des rotations ;
* l’ordre dans lequel effectuer les opérations ;
* la réduction des opérations inutiles ;
* l’adaptation de la stratégie à la taille de l’entrée.
...
Le projet m’a également appris à analyser davantage la **complexité**, à comparer différentes stratégies et à concevoir des structures de données adaptées au problème.
